...
class GFlowNetAgent:
    def __init__(
        ...
        **kwargs,
        ...
        self.ttsr = max(int(optimizer.train_to_sample_ratio), 1)      # Train-to-sample ratio
        self.sttr = max(int(1 / optimizer.train_to_sample_ratio), 1)  # Sample-to-train ratio
    ...
    def estimate_logprobs_data(
        ...
        $log p_T(x) = int_{x in tau} P_F(tau)dtau$
        $= log mathbb{E}_{P_B(tau|x)} frac{P_F(x)}{P_B(tau|x)}$
        $approx log frac{1}{N} sum_{i=1}^{N} frac{P_F(x_i)}{P_B(tau|x_i)}$
        $= log sum_{i=1}^{N} frac{P_F(x_i)}{P_B(tau|x_i)} - log N$
    ...
    def train(self):
        ...
        for it in pbar:
            ...
            self.logger.log_time(times, step=it, use_context=self.use_context)
